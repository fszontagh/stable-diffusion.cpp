...
4. **Block pairing is by descending `norm1` width**, not by module path. With `fusion_blocks="full"` and SD1.5 that's 16 blocks; a stable sort matters (Python's `sorted` is stable, so ties break by DFS order: down_blocks → mid_block → up_blocks).
